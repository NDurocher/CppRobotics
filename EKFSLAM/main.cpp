...
{	
	...
	Eigen::MatrixXd Xest = Eigen::MatrixXd::Zero(slam.state_size,1);
	Eigen::MatrixXd Xtrue = Eigen::MatrixXd::Zero(slam.state_size,1);
	...
	{	
		...
		gp << "plot" << gp.file1d(boost::make_tuple(Est_x, Est_y)) <<  "with lines title 'Estimated Position'," << gp.file1d(boost::make_tuple(True_x, True_y)) \
	 	...
	 	if (SHOW_EST_LM_POS_PLOT) {
			for (int ii = 0; ii < slam.CountLMs(Xest); ii++){
				Est_LM_pos_vec_x.push_back(Xest(slam.state_size + ii * 2,0));
				Est_LM_pos_vec_y.push_back(Xest(slam.state_size + ii * 2 + 1,0));
			}
