...
public:
    PlotVisualizer(int w = 800, int h = 600, int m = 60)
    ...
    cv::Mat plotScatter(const std::vector<float> &x, const std::vector<float> &y,
                        int point_size = 3, cv::Scalar color = cv::Scalar(0, 255, 0)) {
        ...
        for (size_t i = 0; i < x.size(); ++i) {
            int px = margin + ((x[i] - x_min) / (x_max - x_min)) * draw_width;
            int py = margin + draw_height - ((y[i] - y_min) / (y_max - y_min)) * draw_height;
    ...
    cv::Mat plotLine(const std::vector<float> &x, const std::vector<float> &y,
                     int line_width = 2, cv::Scalar color = cv::Scalar(0, 255, 0)) {
        if (x.size() != y.size() || x.empty()) {
            std::cerr << "Invalid data: x and y must have same non-zero size!" << std::endl;
            return cv::Mat::zeros(height, width, CV_8UC3);
        }
    ...
    cv::Mat plotMultiple(const std::vector<std::vector<float> > &x_data,
        ...
        for (size_t s = 0; s < x_data.size(); ++s) {
            ...
            for (size_t i = 0; i < x_data[s].size(); ++i) {
                int px = margin + ((x_data[s][i] - x_min) / (x_max - x_min)) * draw_width;
                int py = margin + draw_height - ((y_data[s][i] - y_min) / (y_max - y_min)) * draw_height;
                points.push_back(cv::Point(px, py));
            }
...
private:
    void drawAxes(cv::Mat &img, float x_min, float x_max, float y_min, float y_max) {
    ...
    void drawLegend(cv::Mat &img, size_t num_series, const std::vector<std::string> &labels) {
        int legend_x = width - margin - 100;
        int legend_y = margin + 20;

        for (size_t i = 0; i < num_series; ++i) {
            cv::Scalar color = colors[i % colors.size()];
            int y_pos = legend_y + i * 20;
