...
namespace cpp_rob {
    ...
    ParticleFilter::ParticleFilter(int num_particles, double noise_x, double noise_y, double noise_theta,
    ...
    std::vector<double> ParticleFilter::updateWeights(std::pair<double, double> measurement, double measurement_std) {
        ...
        for (int ii = 0; ii < particles.size(); ii++) {
            std::vector<double> delta_position{
                robot_position[0] - particles[ii].x, robot_position[1] - particles[ii].y
            };

            double particle_measurement = hypot(measurement.first + delta_position[0],
                                                measurement.second + delta_position[1]);
